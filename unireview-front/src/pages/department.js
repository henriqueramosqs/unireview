...
function Department() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:3001/deptos/${id}`); // Replace with your API endpoint
        if (response.ok) { 
  ...
  const fetchReviews = async () => {
    try {
      const response = await fetch(`http://localhost:3001/reviews?depto_id=${id}`);
      if (response.ok) {
        const data = await response.json();
        setReviews(data);
      } else {
    ...
    <div>
      {data ? (
        <div>
          ...
          <h3>Média de professsores: {data[0].average_professor}</h3>
          <h3>Média das disciplinas: {data[0].average_courses}</h3>
        </div>
      ) : (
        <p>Loading...</p>
      )}
