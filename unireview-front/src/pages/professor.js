...
function Professor() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:3001/profs/${id}`); // Replace with your API endpoint
        if (response.ok) { 
  ...
  const fetchReviews = async () => {
    try {
      const response = await fetch(`http://localhost:3001/reviews?professor_id=${id}`);
      if (response.ok) {
        const data = await response.json();
        setReviews(data);
      } else {
    ...
    <div>
      {data ? (
        <div>
          ...
          <h3>id do depto: {data[0].department_id}</h3>
